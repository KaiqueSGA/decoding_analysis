 ...
 * -------------------
 * Generated by     :: Kaique Yudji
 * Generated at     :: sábado, 25 de março de 2023, 01:15 Hora Coordenada Universal
 * Machine          :: LAPTOP-IUM85ULR <win32> - Node.js v14.17.1
 * Origin file      :: index.js <Javascript>
 * Destination file :: index.tago-io.js
 * -------------------
*/
...
var require_xregexp_all = __commonJS({
  "../node_modules/xregexp/xregexp-all.js"(exports2) {
    var XRegExp;
    ...
    (function(XRegExp2) {
      ...
      XRegExp2.addToken(/\\([pP]){(\^?)([^}]*)}/, function(match, scope) {
    ...
    (function(XRegExp2) {
      ...
      XRegExp2.matchRecursive = function(str, left, right, flags, options) {
        ...
        if (escapeChar) {
          ...
          esc = new RegExp("(?:" + escapeChar + "[\\S\\s]|(?:(?!" + XRegExp2.union([left, right]).source + ")[^" + escapeChar + "])+)+", flags.replace(/[^im]+/g, ""));
        }
        while (true) {
          if (escapeChar) {
...
var require_parser = __commonJS({
  "../node_modules/ftp/lib/parser.js"(exports2, module2) {
...
var require_connection = __commonJS({
  "../node_modules/ftp/lib/connection.js"(exports2, module2) {
    var fs = require("fs");
    var tls = require("tls");
          ...
          self._send(cmd, function(err2, text, code) {
            ...
            if (code === 150 || code === 125) {
              if (isBuffer)
                ...
                fs.stat(input, function(err3, stats) {
                  if (err3)
                    dest.end(input);
                  else
...
// classes/ftp_and_tago_functions.js
var require_ftp_and_tago_functions = __commonJS({
  "classes/ftp_and_tago_functions.js"(exports2, module2) {
    var axios = require("axios");
    var ftp_and_tago_function = class {
      constructor(xml_file_name, ftp_connection) {
        __publicField(this, "xml_file_name");
        __publicField(this, "ftp_connection");
        __publicField(this, "file_content");
          ...
          this.ftp_connection.get(`./${this.xml_file_name}`, async function(err, stream) {
            if (err) {
              ...
              await stream.on("data", (chunk) => {
                ...
                if (!file_content.includes("</payload>")) {
                  resolve(void 0);
                } else {
                  let stu_messages = file_content !== void 0 ? file_content.split("</stuMessage>") : void 0;
                  stu_messages !== void 0 && stu_messages.pop();
      ...
      async insert_on_tago(decoded_code, account_tago, Device2, device_id, stu_message, time) {
        let device_token = (await account_tago.devices.paramList(device_id)).find((parameter) => parameter.key === "device_token").value;
        const tago_device = new Device2({ token: device_token });
        decoded_code.location !== void 0 && this.add_google_link(decoded_code);
        decoded_code.location !== void 0 && await this.add_google_address(decoded_code);
      ...
      async add_google_address(decoded_code) {
        ...
        if (request.data.address !== void 0) {
          let address = request.data.address;
          return address.quarter === void 0 ? decoded_code.metadata.address = `${address.road} - ${address.suburb} - ${address.state} - ${address.city} - ${address.postcode}` : decoded_code.metadata.address = `${address.road} - ${address.quarter} - ${address.state} - ${address.city} - ${address.postcode}`;
        } else {
          decoded_code.metadata.address = "no data available";
        }
      }
    };
    module2.exports = ftp_and_tago_function;
  }
});

// classes/stx.js
var require_stx = __commonJS({
  "classes/stx.js"(exports2, module2) {
    var ftp_and_tago_function = require_ftp_and_tago_functions();
    var stx_message = class extends ftp_and_tago_function {
      constructor(xml_file_name, ftp_connection) {
        super(xml_file_name, ftp_connection);
        __publicField(this, "catch_payload", (stu_message) => {
        ...
        __publicField(this, "hex_2_bin", (hexadecimal_content) => {
          return ("00000000" + parseInt(hexadecimal_content, 16).toString(2)).substr(-8);
        });
        __publicField(this, "decode_lat", (file_content, cardinal_point) => {
          let hexadecimal_lat = file_content.substring(0, 6);
          let lat = String(parseInt(hexadecimal_lat, 16));
          let first_2_numbers_of_coordinate = cardinal_point.cardinal_position === "south" ? "-" + lat.substring(0, 2) : lat.substring(0, 2);
          let rest_of_coordinate = lat.substring(2);
          return first_2_numbers_of_coordinate + "." + rest_of_coordinate;
        });
        __publicField(this, "decode_lng", (file_content, cardinal_point) => {
          let hexadecimal_lng = file_content.substring(6, 12);
          let lng = String(parseInt(hexadecimal_lng, 16));
          let first_2_numbers_of_coordinate = cardinal_point.cardinal_position === "weast" ? "-" + lng.substring(0, 2) : lng.substring(0, 2);
          let rest_of_coordinate = lng.substring(2);
          return first_2_numbers_of_coordinate + "." + rest_of_coordinate;
        });
        __publicField(this, "decode_binary_values", (file_content) => {
          let object_array = new Array();
          let binary = this.hex_2_bin(file_content.substring(12, 14));
          let value_of_each_byte = {
            "0": (byte) => {
              byte === "0" ? object_array.push({ cardinal_position: "south" }) : object_array.push({ cardinal_position: "north" });
            },
            "1": (byte) => {
              byte === "0" ? object_array.push({ cardinal_position: "weast" }) : object_array.push({ cardinal_point: "east" });
            },
            "2": (byte) => {
              byte === "0" ? object_array.push({ origin: "GPS" }) : object_array.push({ origin: "GPS-DR" });
            },
            "3": (byte) => {
              byte === "0" ? object_array.push({ mode: 2 }) : object_array.push({ mode: "jamming" });
            }
          };
          for (let i = 0; i <= 3; i++) {
    ...
    module2.exports = stx_message;
  }
});

// classes/soc.js
var require_soc = __commonJS({
  "classes/soc.js"(exports2, module2) {
    var ftp_and_tago_function = require_ftp_and_tago_functions();
    var smart_one_c_message2 = class extends ftp_and_tago_function {
      constructor(xml_file_name, ftp_connection) {
        super(xml_file_name, ftp_connection);
      }
      decode(file_content, esn_value) {
        ...
        if (byte_that_countains_the_type_of_message === "02") {
          console.log("raw message", byte_that_countains_the_type_of_message);
        } else if (hex_2_bin[7] === "0") {
          console.log("default message (atlasTrax)", byte_that_countains_the_type_of_message);
          return this.Decode_default_message(hexa_code, esn_value, catch_unix_time());
        } else if (hex_2_bin[7] === "1") {
          console.log("type3", byte_that_countains_the_type_of_message);
          return this.Decode_type3_message(hexa_code, esn_value, catch_unix_time());
        }
      }
      Decode_default_message(hexa_code, esn_value, unix_time) {
        const latitude = this.decode_lat(hexa_code.substring(2, 8));
        const longitude = this.decode_lng(hexa_code.substring(8, 14));
        let object_with_datas_to_insert_on_tago = { variable: "ESN", value: esn_value, location: { type: "Point", coordinates: [longitude, latitude] }, metadata: { message_type: 0, device_type: "SOC" } };
        let current_byte = "";
        let byte_array = new Array();
        let value_of_each_bit = {
          "00": (binary_value) => {
            return { message_type: binary_value };
          },
          "01": (binary_value) => {
            return { input_2: binary_value };
          },
          "02": (binary_value) => {
            return { input_1: binary_value };
          },
          "03": (binary_value) => {
            return { external_power: binary_value };
          },
          "04": (binary_value) => {
            return { vibration: binary_value };
          },
          "07": (current_byte_2_bin) => {
            let cardinal_direction = parseInt(current_byte_2_bin.substring(5), 2);
            return { bearing: cardinal_direction };
          },
          "70": (current_byte_2_bin) => {
            return { speed: parseInt(current_byte_2_bin, 2) };
          },
          "86": (current_byte_2_bin) => {
            let gps_value = parseInt(current_byte_2_bin.substring(1, 8), 2);
            let utc_time = new Date(unix_time * 1e3);
            ...
            let gps_time = (gw_chunk * 720 + gps_seconds) * 1e3;
            ...
            if (result_date_time > utc_time)
              result_date_time = utc_time;
            return { time: result_date_time };
          },
          "87": (binary_value) => {
            return { batery_change: binary_value };
          }
        };
        for (let i = 0; i < hexa_code.length; i++) {
          ...
          if (current_byte.length === 2) {
            ...
            const decode_binary_code = (() => {
              for (let i2 = 0; i2 < current_byte_2_bin.length; i2++) {
                let values = `${byte_array.indexOf(current_byte)}${i2}`;
                values !== "00" && (() => {
                  let bit_value = typeof value_of_each_bit[values] === "function" ? value_of_each_bit[values](values === "07" || values === "70" || values === "86" ? current_byte_2_bin : Number(current_byte_2_bin.split("").reverse().join("")[i2])) : void 0;
                  if (bit_value !== void 0) {
      ...
      Decode_truncate_message(hexa_code, esn_value) {
      ...
      Decode_type3_message(hexa_code, esn_value) {
        ...
        if (subtype()) {
          if (subtype() === "DIAGNOSTIC MESSAGE") {
            ...
            console.log("replace");
            return decode_diagnostic_message("bateryMessage");
        ...
        function decode_diagnostic_message(subtype2) {
          let byte_array = new Array();
          let current_byte = "";
          for (let i = 0; i < hexa_code.length; i++) {
            current_byte += hexa_code[i];
            if (current_byte.length === 2) {
              byte_array.push(current_byte);
              let current_byte_2_bin = ("00000000" + parseInt(current_byte, 16).toString(2)).slice(-8);
              let value_of_each_bit = { "103": () => {
                return { number_of_transmissions: parseInt(current_byte_2_bin[0] + current_byte_2_bin[1] + current_byte_2_bin[2] + current_byte_2_bin[3], 2) };
              }, "113": () => {
                return { number_of_transmissions: parseInt(current_byte_2_bin[0] + current_byte_2_bin[1] + current_byte_2_bin[2] + current_byte_2_bin[3], 2) };
              }, "104": { battery_condition: "Good batery" }, "114": { battery_condition: "Replace batery" }, "105": { gps_subsystem_fault: "GPS system OK." }, "115": { gps_subsystem_fault: "Fault" }, "106": { transmitter_subsystem_fault: "Trasnmitter OK." }, "116": { transmitter_subsystem_fault: "Fault" }, "107": { scheduler_subsystem_fault: "Transmitter OK." }, "117": { scheduler_subsystem_fault: "Fault" }, "2": () => {
                return { min_interval: parseInt(current_byte_2_bin, 2) };
              }, "3": () => {
                return { max_interval: parseInt(current_byte_2_bin, 2) };
              }, "4": () => {
                return { gps_mean_search_time: parseInt(current_byte_2_bin, 2) };
              }, "6": () => {
                let byte5_2_bin = ("00000000" + parseInt(byte_array[5], 16).toString(2)).slice(-8);
                let byte6_to_bin = ("00000000" + parseInt(byte_array[6], 16).toString(2)).slice(-8);
                return { gps_fails: parseInt(byte5_2_bin, 2) + parseInt(byte6_to_bin, 2) };
              }, "8": () => {
                let byte7_2_bin = ("00000000" + parseInt(byte_array[7], 16).toString(2)).slice(-8);
                let byte8_to_bin = ("00000000" + parseInt(byte_array[8], 16).toString(2)).slice(-8);
                return { transmission_numbers: parseInt(byte7_2_bin, 2) + parseInt(byte8_to_bin, 2) };
              } };
              if (byte_array.indexOf(current_byte) !== 0) {
                if (byte_array.indexOf(current_byte) === 1) {
                  for (let i2 = 0; i2 < current_byte_2_bin.length; i2++) {
                    let values = `${byte_array.indexOf(current_byte)}${current_byte_2_bin[i2]}${i2}`;
                  ...
                  Object.assign(object_with_datas_to_insert_on_tago.metadata, value_of_each_bit["6"]());
          ...
          subtype2 === "diagnosticMessage" ? object_with_datas_to_insert_on_tago.metadata.sub_type = "diagnosticMessage" : object_with_datas_to_insert_on_tago.metadata.sub_type = "bateryMessage";
          return object_with_datas_to_insert_on_tago;
        }
        function AccumulateCountMessage() {
          ...
          for (let i = 0; i < hexa_code.length; i++) {
            ...
            if (current_byte.length === 2) {
              byte_array.push(current_byte);
              let value_of_each_bit = { "1": () => {
                if (hexa_code.substring(2, 4) === "FFFF") {
                  return { entry_1_stored: "Turn entry 1 storage off" };
                } else {
                  return { entry_1_stored: parseInt(hexa_code.substring(0, 4), 16) * 10 };
                }
              }, "3": () => {
                if (hexa_code.substring(2, 4) === "FFFF") {
                  return { entry_2_stored: "Turn entry 1 storage off" };
                } else {
                  return { entry_2_stored: parseInt(hexa_code.substring(4, 8), 16) * 10 };
                }
              }, "5": { storage_vibration: parseInt(hexa_code.substring(8, 12), 16) * 10 }, "7": () => {
                if (hexa_code.substring(0, 2) === "FF") {
                  return { entry_count_1: "off" };
                } else {
                  return { entry_count_1: parseInt(hexa_code.substring(12, 14), 16) };
                }
              }, "8": () => {
                if (hexa_code.substring(0, 2) === "FF") {
                  return { entry_count_2: "off" };
                } else {
                  return { entry_count_2: parseInt(hexa_code.substring(14, 16), 16) };
                }
              } };
              ...
              byte_array.indexOf(current_byte) === 1 ? Object.assign(object_with_datas_to_insert_on_tago.metadata, bit_value) : byte_array.indexOf(current_byte) === 3 ? Object.assign(object_with_datas_to_insert_on_tago.metadata, bit_value) : byte_array.indexOf(current_byte) === 5 ? Object.assign(object_with_datas_to_insert_on_tago.metadata, bit_value) : byte_array.indexOf(current_byte) === 7 ? Object.assign(object_with_datas_to_insert_on_tago.metadata, bit_value) : byte_array.indexOf(current_byte) === 8 && Object.assign(object_with_datas_to_insert_on_tago.metadata, bit_value);
              current_byte = "";
            }
          }
        ...
        function subtype() {
          ...
          let response;
          parseInt(hex_2_bin.substring(2, 8), 2) === 21 && (() => {
            response = "DIAGNOSTIC MESSAGE";
          })();
          parseInt(hex_2_bin.substring(2, 8), 2) === 22 && (() => {
            response = "REPLACE BATERY";
          })();
          parseInt(hex_2_bin.substring(2, 8), 2) === 23 && (() => {
            response = "CONTACT SERVICE PROVIDER MESSAGE";
          })();
          parseInt(hex_2_bin.substring(2, 8), 2) === 24 && (() => {
            response = "ACCUMULATE/COUNT MESSAGE";
          })();
          return response;
        }
      }
      decode_lat(latitude_hexadecimal_format) {
        let hex_2_decimal = parseInt(latitude_hexadecimal_format, 16);
        let degree_per_count_lat = 90 / Math.pow(2, 23);
        let latitude = hex_2_decimal * degree_per_count_lat > 90 ? hex_2_decimal * degree_per_count_lat - 180 : hex_2_decimal * degree_per_count_lat;
        return latitude;
      }
      decode_lng(longitude_hexadecimal_format) {
        let hex_2_decimal = parseInt(longitude_hexadecimal_format, 16);
        let degree_per_count_long = 180 / Math.pow(2, 23);
        let longitude = hex_2_decimal * degree_per_count_long > 180 ? hex_2_decimal * degree_per_count_long - 360 : hex_2_decimal * degree_per_count_long;
        return longitude;
      }
      get_hexa_code_from_ftp_file(stu_message) {
        try {
          let firstTag = stu_message.indexOf(">", stu_message.indexOf("<payload"));
          let secondTag = stu_message.indexOf("</payload>", firstTag);
          return stu_message.substring(firstTag + 3, secondTag);
    ...
    module2.exports = smart_one_c_message2;
  }
});

// classes/Apis/location.js
var require_location = __commonJS({
  "classes/Apis/location.js"(exports2, module2) {
    var axios = require("axios");
    var location_apis = class {
      constructor() {
        __publicField(this, "prepare_mac_parameterers_for_requisition", (wifi_fields, scope) => {
          let wifi_parameters_list = new Array();
          if (wifi_fields === void 0) {
            return;
          } else {
            wifi_fields.forEach((key) => {
              if (!scope[0].metadata[key]) {
                return;
              }
        ...
        __publicField(this, "parseToObjectLBS", (keyTag, scope, dataType) => {
          if (keyTag === void 0) {
            return;
          }
        ...
        __publicField(this, "get_address_through_coordinates", async (latitude, longitude) => {
          const request = await axios.get(`https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${latitude}&lon=${longitude}`);
          if (request.data.address !== void 0) {
            let address = request.data.address;
            return address.quarter === void 0 ? `${address.road} - ${address.suburb} - ${address.state} - ${address.city} - ${address.postcode}` : `${address.road} - ${address.quarter} - ${address.state} - ${address.city} - ${address.postcode}`;
          } else {
            return { address: "no data available" };
          }
        });
        __publicField(this, "get_coordinates_through_mac_datas", async (wifi_fields, scope) => {
        ...
        __publicField(this, "get_coordinates_through_lbs_datas", async (list) => {
          try {
            ...
            const result = await axios({
              ...
              data: {
                ...
                radioType: mode.endsWith("/lte") === true ? "LTE" : "gsm",
                carrier: "Vodafone",
                considerIp: false,
    ...
    module2.exports = location_apis;
  }
});

// classes/mqtt.js
var require_mqtt = __commonJS({
  "classes/mqtt.js"(exports2, module2) {
    var ftp_and_tago_function = require_ftp_and_tago_functions();
    var location_api = require_location();
    var mqtt_message = class {
      constructor(esn2) {
        __publicField(this, "mapLINK", "https://www.google.com/maps/search/?api=1&query=");
        __publicField(this, "esn");
        __publicField(this, "add_battery_field", (tmpSTR) => {
          tmpSTR = tmpSTR.replace(":", ",");
          var tmpSPLIT = tmpSTR.split(",");
          var tmpFLOAT = tmpSPLIT[3];
          tmpFLOAT = tmpFLOAT / 1e3;
          this.esn.metadata.battery = tmpFLOAT;
        });
        __publicField(this, "add_operator_field", (tmpSTR) => {
          tmpSTR = tmpSTR.replace("cops:", "");
          tmpSTR = tmpSTR.trim();
          this.esn.metadata.cops = tmpSTR;
        });
        __publicField(this, "add_jamming_state", (tmpSTR) => {
          tmpSTR = tmpSTR.replace("sjdr:", "");
          tmpSTR = tmpSTR.trim();
          this.esn.metadata.jamm = tmpSTR;
        });
        __publicField(this, "get_cardinal_direction", (tmpFLOAT) => {
          var tmpDIR = "NULL";
          if (tmpFLOAT >= 360 - 45 / 2 && tmpFLOAT <= 360 + 45 / 2)
            tmpDIR = "N";
          if (tmpFLOAT >= 0 - 45 / 2 && tmpFLOAT <= 0 + 45 / 2)
            tmpDIR = "N";
          if (tmpFLOAT >= 45 - 45 / 2 && tmpFLOAT <= 45 + 45 / 2)
            tmpDIR = "NE";
          if (tmpFLOAT >= 90 - 45 / 2 && tmpFLOAT <= 90 + 45 / 2)
            tmpDIR = "E";
          if (tmpFLOAT >= 135 - 45 / 2 && tmpFLOAT <= 135 + 45 / 2)
            tmpDIR = "SE";
          if (tmpFLOAT >= 180 - 45 / 2 && tmpFLOAT <= 180 + 45 / 2)
            tmpDIR = "S";
          if (tmpFLOAT >= 225 - 45 / 2 && tmpFLOAT <= 225 + 45 / 2)
            tmpDIR = "SW";
          if (tmpFLOAT >= 270 - 45 / 2 && tmpFLOAT <= 270 + 45 / 2)
            tmpDIR = "W";
          if (tmpFLOAT >= 315 - 45 / 2 && tmpFLOAT <= 315 + 45 / 2)
            tmpDIR = "NW";
          return tmpDIR;
        });
        __publicField(this, "fncGNS", (tmpSTR) => {
          tmpSTR = tmpSTR.replace("*", ",");
          tmpSTR = tmpSTR.replace(":", ",");
          var tmpSPLIT = tmpSTR.split(",");
          ...
          if (tmpFIX == 0)
            return;
          if (tmpFIX == 1)
            esn.metadata.origin = "GPS";
          if (tmpFIX == 2)
            esn.metadata.origin = "GPS-DR";
          this.esn.location = {};
          ...
          this.esn.metadata.lat = tmpSPLIT[4];
          this.esn.metadata.lon = tmpSPLIT[5];
          this.esn.metadata.alt = tmpSPLIT[6];
          this.esn.metadata.spd = tmpSPLIT[7];
          this.esn.metadata.cog = tmpSPLIT[8];
          var tmpCOG = esn.metadata.cog;
          var tmpFLOAT = 0;
          var tmpDIR = "";
        ...
        __publicField(this, "fncGPS166", (tmpSTR) => {
          ...
          tmpFLOAT = tmpFLOAT + tmpINT;
          tmpSTR = String(tmpFLOAT.toFixed(8));
          return tmpSTR;
        });
        __publicField(this, "fncRMC", (tmpSTR) => {
          tmpSTR = tmpSTR.replace("*", ",");
          var tmpSPLIT = tmpSTR.split(",");
        ...
        __publicField(this, "fncGLL", (tmpSTR) => {
          tmpSTR = tmpSTR.replace("*", ",");
          var tmpSPLIT = tmpSTR.split(",");
          if (tmpSPLIT[6] == "V")
            return;
        ...
        __publicField(this, "fncGGA", (tmpSTR) => {
          tmpSTR = tmpSTR.replace("*", ",");
          var tmpSPLIT = tmpSTR.split(",");
          if (tmpSPLIT[6] == "0")
            return;
        ...
        __publicField(this, "get_rtc_time", (tmpSTR) => {
          tmpSTR = tmpSTR.replace("*", ",");
          var tmpSPLIT = tmpSTR.split(",");
        ...
        __publicField(this, "fncVTG", (tmpSTR) => {
          tmpSTR = tmpSTR.replace("*", ",");
          var tmpSPLIT = tmpSTR.split(",");
          if (tmpSPLIT[9] == "N")
            return;
        ...
        __publicField(this, "get_message_origin", (tmpSTR) => {
          tmpSTR = tmpSTR.replace("*", ",");
          var tmpSPLIT = tmpSTR.split(",");
          tmpSTR = "NULL";
          if (tmpSPLIT[6] == "A")
        ...
        __publicField(this, "decode", async (scope) => {
          var values_array = this.esn.value.split(";");
          this.esn.variable = values_array[0];
          this.esn.value = values_array[1];
          for (let i = 2; i < values_array.length; i++) {
            ...
            if (field_key == "EOF") {
              i = values_array.length;
            }
            ;
            this.esn.metadata[field_key.toLowerCase()] = field_value;
            if (field_key.startsWith("battery_volts")) {
              this.add_battery_field(field_value);
            } else if (field_key.startsWith("cops")) {
              this.add_operator_field(field_value);
            } else if (field_key.startsWith("jamm")) {
              this.add_jamming_state(field_value);
            } else if (field_key.startsWith("gns")) {
              this.fncGNS(field_value);
            } else if (field_key.startsWith("rmc")) {
              this.fncRMC(field_value);
            } else if (field_key.startsWith("gll")) {
              this.fncGLL(field_value);
            } else if (field_key.startsWith("gga")) {
              this.fncGGA(field_value);
            } else if (field_key.startsWith("zda")) {
              this.get_rtc_time(field_value);
            } else if (field_key.startsWith("vtg")) {
              this.fncVTG(field_value);
            } else if (field_key.startsWith("psti20")) {
              this.get_message_origin(field_value);
            }
          }
          delete this.esn.metadata.mqtt_topic;
          delete this.esn.metadata.EOF;
          if (this.esn.metadata.RTC) {
            this.esn.time = this.esn.metadata.RTC;
          }
          ;
          if (this.esn.location !== void 0) {
            if (typeof this.esn.location.lat === "number" && typeof this.esn.location.lng === "number") {
    ...
    module2.exports = mqtt_message;
  }
});

// classes/Apis/tago.js
var require_tago = __commonJS({
  "classes/Apis/tago.js"(exports2, module2) {
    var tago_functions2 = class {
      constructor(account) {
        __publicField(this, "account");
        this.account = account;
      }
      async insert_on_tago(decoded_code, Device2, device_id, time) {
    ...
    module2.exports = tago_functions2;
  }
});

// index.js
var { Analysis, Account, Utils, Services, Device } = require("@tago-io/sdk");
var Ftp_server = require_connection();
var external_functions = require_ftp_and_tago_functions();
var stx_messages = require_stx();
var soc_messages = require_soc();
var mqtt_messages = require_mqtt();
var tago_functions = require_tago();
async function Changing_algorithm(file_list, ftp_connection, account_tago) {
  ...
  const catch_time_stamp = (data) => {
    ...
    let date_time_elements = time_stamp.split(" ");
    let date_elements = date_time_elements[0].split("/");
    let time_elements = date_time_elements[1].split(":");
  ...
  for await (let ftp_file of file_list) {
    try {
      ...
      const ftp_functions = new external_functions(ftp_file.name, ftp_connection);
      let file_content = await ftp_functions.get_file_content();
      ...
      for await (let stu_message of file_content) {
        ...
        if (device[0].tags.find((tag) => tag.key === "TYPE" && tag.value === "SOC")) {
          console.log("SOC");
          const smart_one_c_message2 = new soc_messages(ftp_file.name, ftp_connection);
          let decoded_code;
          decoded_code = smart_one_c_message2.decode(stu_message, esn_value);
          decoded_code !== void 0 && await smart_one_c_message2.insert_on_tago(decoded_code, account_tago, Device, device[0].id, stu_message);
          decoded_code !== void 0 && await smart_one_c_message2.delete_file_from_ftp();
        } else if (device[0].tags.find((tag) => tag.key === "TYPE" && tag.value === "STX")) {
          console.log("STX");
          const stx_message = new stx_messages(ftp_file.name, ftp_connection);
          let decoded_code;
          decoded_code = stx_message.decode(stu_message, esn_value);
          decoded_code !== void 0 && await stx_message.insert_on_tago(decoded_code, account_tago, Device, device[0].id, stu_message, time_stamp);
          decoded_code !== void 0 && await stx_message.delete_file_from_ftp();
        } else {
        }
      }
      ;
  ...
  try {
    ...
    if (scope.length !== 0) {
      ...
      const identify_device_on_tago = async () => {
        let esn2 = scope[0].value.split(";")[1];
        let filter = { tags: [{ key: "ESN", value: esn2 }] };
      ...
      decoded_code !== void 0 && await tago_func.insert_on_tago(decoded_code, Device, device_id[0].id, decoded_code);
      return;
    }
