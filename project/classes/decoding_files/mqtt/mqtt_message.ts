...
import { gps_sentences } from "../mqtt/gps_sentences";
  ...
  export default async function decode(scope: any){
    ...
    for (let i = 2; i < values_array.length; i++) {
        ...
        var field_value: string = values_array[i].replace(field_key + ",", "").trim();
    ...
    let rmc_value: any | undefined = values_array.find(item => item.includes("rmc"));
     if(rmc_value !== undefined){
      ...
      await gps_sentence.decode_rmc(field_value);
        ...
        const map_link: string = "https://www.google.com/maps/search/?api=1&query=";

        var mac_coordinates: any = gps_sentence.new_variable_to_be_inserted.metadata.mac0 ?await location_functions.get_coordinates_through_mac_datas( ["mac0", "mac1", "mac2"], gps_sentence.new_variable_to_be_inserted ) :{lat:0, lng:0};
        var lbs_coordinates: any = gps_sentence.new_variable_to_be_inserted.metadata.lbs0 ?await location_functions.get_coordinates_through_lbs_datas( ["lbs0", "lbs1", "lbs2"], gps_sentence.new_variable_to_be_inserted, gps_sentence.new_variable_to_be_inserted.metadata.lbs_mode === "LTE" ?"lte" :"gsm" ) :{lat:0, lng:0}; 
