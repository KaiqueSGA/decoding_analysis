...
const cacth_esn = (data) => {
  //what is ESN? Read in our README.
...
const catch_time_stamp = (data) => {
  ...
  time_stamp = new Date(
    `${date_elements[2]}-${date_elements[1]}-${date_elements[0]}T${time_elements[0]}:${time_elements[1]}:${time_elements[2]}Z`
  );
  return time_stamp;
};
  ...
  for await (let ftp_file of file_list) {
    try {
      ...
      for await (let stu_message of file_content) {
        // I need to do this for because inside of file content, i can have more than one message. The file content can has many stu_messages, therefore i need of more one loopig
        ...
        if (
          device[0].tags.find(
            (tag) => tag.key === "TYPE" && tag.value === "SOC"
          )
        ) {
          console.log("SOC");
          ...
          decoded_code !== undefined &&
            (await tago_function.insert_on_tago(
              decoded_code,
              Device,
              device[0].id
            ));
          ...
          device[0].tags.find(
            (tag) => tag.key === "TYPE" && tag.value === "STX"
          )
        ) {
          console.log("STX");
          ...
          decoded_code = await stx_message.decode(
            stu_message,
            esn_value,
            time_stamp
          );
      ...
      console.log(`Something went wrong  in the file${ftp_file.name}...`, err);
      continue;
    }
  }
    ...
    connection.on("ready", function () {
      connection.list("*.xml", function (err, file_list) {
        ...
        if (file_list.length === 0) {
          connection.destroy();
          console.log("WITHOUT files in FTP");
          process.kill(process.pid, "SIGINT");
        } else {
...
module.exports = new Analysis(Decoding_analysis, {
  token: "67937c9b-f516-448c-aad2-c6369fbf8e7a",
});
