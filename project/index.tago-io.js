 ...
 * -------------------
 * Generated by     :: kaique
 * Generated at     :: sexta-feira, 8 de setembro de 2023, 17:35 Hora Coordenada Universal
 * Machine          :: DESKTOP-67MU0D9 <win32> - Node.js v16.16.0
 * Origin file      :: index.ts <Typescript>
...
var stx_message = class {
  ...
  decode_lat(file_content, cardinal_position) {
    ...
    let integer_lat = parseInt(hexadecimal_lat, 16);
  ...
  decode_binary_values(file_content) {
    ...
    value_of_each_byte[0] = (byte) => {
      byte === "0" ? values_object.cardinal_position_s_n = "south" : values_object.cardinal_position_s_n = "north";
    };
    value_of_each_byte[1] = (byte) => {
      byte === "0" ? values_object.cardinal_position_w_e = "weast" : values_object.cardinal_position_w_e = "east";
    };
    value_of_each_byte[2] = (byte) => {
      byte === "0" ? values_object.origin = "GPS" : values_object.origin = "GPS-DR";
    };
    value_of_each_byte[3] = (byte) => {
      byte === "0" ? values_object.mode = 2 : values_object.mode = 3;
    };
    for (let i = 0; i <= 3; i++) {
      value_of_each_byte[i](binary[i]);
    }
    if (binary.substring(5) === "000") {
      values_object.direction = "N";
    } else if (binary.substring(5) === "001") {
      values_object.direction = "NE";
    } else if (binary.substring(5) === "010") {
      values_object.direction = "E";
    } else if (binary.substring(5) === "011") {
      values_object.direction = "SE";
    } else if (binary.substring(5) === "100") {
      values_object.direction = "S";
    } else if (binary.substring(5) === "101") {
      values_object.direction = "SW";
    } else if (binary.substring(5) === "110") {
      values_object.direction = "W";
    } else if (binary.substring(5) === "111") {
      values_object.direction = "NW";
    }
...
// classes/Apis/tago.ts
var tago_functions = class {
  constructor(account) {
...
// classes/decoding_files/mqtt/gps_sentences.ts
var gps_sentences = class {
  constructor(new_variable_to_be_inserted) {
    this.coordinates_origin = "";
    this.mapLINK = "https://www.google.com/maps/search/?api=1&query=";
    this.new_variable_to_be_inserted = new_variable_to_be_inserted;
  }
  decode_gns(sentence_value) {
    sentence_value = sentence_value.replace("*", ",");
    sentence_value = sentence_value.replace(":", ",");
    var values_array = sentence_value.split(",");
    var origin_field = values_array[9];
    if (origin_field == "0") {
      return;
    }
    ;
    if (origin_field == "1") {
      this.new_variable_to_be_inserted.metadata.origin = "GPS";
    }
    if (origin_field == "2") {
      this.new_variable_to_be_inserted.metadata.origin = "GPS-DR";
    }
    ...
    var geography_coordinates = Number(values_array[4] + "," + values_array[5]);
    ...
    this.new_variable_to_be_inserted.metadata.lat = values_array[4];
    this.new_variable_to_be_inserted.metadata.lon = values_array[5];
    this.new_variable_to_be_inserted.metadata.alt = values_array[6];
    this.new_variable_to_be_inserted.metadata.spd = values_array[7];
    this.new_variable_to_be_inserted.metadata.cog = values_array[8];
  ...
  transform_raw_data_into_coordinates(sentence_value) {
    let raw_coordinate = parseFloat(sentence_value);
    raw_coordinate = raw_coordinate / 100;
    let coordinate_integer_value = parseInt(String(raw_coordinate));
    raw_coordinate = raw_coordinate - coordinate_integer_value;
    raw_coordinate = raw_coordinate / 0.6;
    raw_coordinate = raw_coordinate + coordinate_integer_value;
    sentence_value = String(raw_coordinate.toFixed(8));
    return sentence_value;
  }
  async decode_rmc(sentence_value) {
    let location_functions = new location_apis();
    sentence_value = sentence_value.replace("*", ",");
    let values_array = sentence_value.split(",");
    let mac_coordinates = this.new_variable_to_be_inserted.metadata.mac0 ? await location_functions.get_coordinates_through_mac_datas(["mac0", "mac1", "mac2"], this.new_variable_to_be_inserted) : { lat: 0, lng: 0 };
    let lbs_coordinates = this.new_variable_to_be_inserted.metadata.lbs0 ? await location_functions.get_coordinates_through_lbs_datas(["lbs0", "lbs1", "lbs2"], this.new_variable_to_be_inserted, this.new_variable_to_be_inserted.metadata.lbs_mode === "LTE" ? "lte" : "gsm") : { lat: 0, lng: 0 };
    ...
    if (values_array[12] == "N") {
      ...
      if (coordinates.lat === 0 || coordinates.lng === 0) {
        let latitude = values_array[3];
        let longitude = values_array[5];
        let tmpNS = values_array[4];
        var tmpEW = values_array[6];
      ...
      let knot = 1.852;
      let speed = (parseInt(values_array[7]) * knot).toFixed(1);
      let cog = parseInt(values_array[8]);
      let direction = this.get_cardinal_direction(cog);
      let tmpIEC = values_array[13];
      ...
      let north_south = values_array[4];
      let east_west = values_array[6];
      let latitude = this.transform_raw_data_into_coordinates(values_array[3]);
      if (north_south == "S")
        latitude = "-" + latitude;
      let longitude = this.transform_raw_data_into_coordinates(values_array[5]);
      if (east_west == "W")
        longitude = "-" + longitude;
      let knot = 1.852;
      let speed = (parseInt(values_array[7]) * knot).toFixed(1);
      ...
      let geography_coordinates = latitude + "," + longitude;
      let tmpLINK = this.mapLINK + geography_coordinates;
      this.new_variable_to_be_inserted.metadata.url_pin = {};
  ...
  decode_gll(sentence_value) {
    sentence_value = sentence_value.replace("*", ",");
    let array_values = sentence_value.split(",");
    if (array_values[6] == "V")
      return;
    if (array_values[7] == "N")
      return;
    var north_south = array_values[2];
    let latitude = this.transform_raw_data_into_coordinates(array_values[1]);
    if (north_south == "S") {
      latitude = "-" + latitude;
    }
    let east_west = array_values[4];
    let longitude = this.transform_raw_data_into_coordinates(array_values[3]);
    if (east_west == "W") {
      longitude = "-" + longitude;
    }
    this.new_variable_to_be_inserted.location = {};
    ...
    var geography_coordinates = latitude + "," + longitude;
    var tmpLINK = this.mapLINK + geography_coordinates;
    this.new_variable_to_be_inserted.metadata.url_pin = {};
    this.new_variable_to_be_inserted.metadata.url_pin.url = tmpLINK;
    this.new_variable_to_be_inserted.metadata.url_pin.alias = "Open map at " + geography_coordinates;
    this.new_variable_to_be_inserted.metadata.link = tmpLINK;
  ...
  decode_gga(sentence_value) {
    sentence_value = sentence_value.replace("*", ",");
    var array_values = sentence_value.split(",");
    if (array_values[6] == "0")
      return;
    let latitude = this.transform_raw_data_into_coordinates(array_values[2]);
    let north_south = array_values[3];
    if (north_south == "S") {
      latitude = "-" + latitude;
    }
    var longitude = this.transform_raw_data_into_coordinates(array_values[4]);
    var east_west = array_values[5];
    if (east_west == "W")
      longitude = "-" + longitude;
    let tmpALT = array_values[9];
    this.new_variable_to_be_inserted.location = {};
    this.new_variable_to_be_inserted.location.lat = parseFloat(latitude);
    this.new_variable_to_be_inserted.location.lng = parseFloat(longitude);
    var geography_coordinates = latitude + "," + longitude;
  ...
  decode_zda(sentence_value) {
    sentence_value = sentence_value.replace("*", ",");
    var array_values = sentence_value.split(",");
    var strDD = array_values[2];
    var strMM = array_values[3];
    var strYY = array_values[4];
    var strH = array_values[1].substring(0, 2);
    var strM = array_values[1].substring(2, 4);
    var strS = array_values[1].substring(4, 6);
    var date = strYY + "-" + strMM + "-" + strDD;
    var time = strH + ":" + strM + ":" + strS;
    var date_time = date + " " + time;
    this.new_variable_to_be_inserted.metadata.zda_time = date_time;
  }
  decode_vtg(sentence_value) {
    sentence_value = sentence_value.replace("*", ",");
    var array_values = sentence_value.split(",");
    var speed = array_values[7];
    var tmpCOG = array_values[1];
    let direction = this.get_cardinal_direction(parseInt(tmpCOG));
    this.new_variable_to_be_inserted.metadata.spd = speed;
    this.new_variable_to_be_inserted.metadata.cog = tmpCOG;
    this.new_variable_to_be_inserted.metadata.direction = direction;
  }
  decode_psti20(sentence_value) {
    let dr_cal = sentence_value.split(",")[2];
    let gyro_cal = sentence_value.split(",")[3];
    sentence_value = sentence_value.replace("*", ",");
    var array_values = sentence_value.split(",");
    sentence_value = "null";
    if (array_values[6] == "A")
      sentence_value = "GPS";
    if (array_values[6] == "E")
      sentence_value = "GPS-DR";
    if (array_values[6] == "N")
      sentence_value = "ERROR";
    if (sentence_value === "null" || sentence_value === "ERROR") {
      sentence_value = this.coordinates_origin;
    }
  ...
  add_battery_field(sentence_value) {
    sentence_value = sentence_value.replace(":", ",");
    let array_values = sentence_value.split(",");
    let battery = parseInt(array_values[3]);
    battery = battery / 1e3;
    this.new_variable_to_be_inserted.metadata.battery = battery;
  }
  add_operator_field(sentence_value) {
  ...
  add_jamming_state(sentence_value) {
...
// classes/decoding_files/mqtt/mqtt_message.ts
async function decode(scope) {
  var values_array = scope[0].value.split(";");
  ...
  for (let i = 2; i < values_array.length; i++) {
    ...
    if (field_key.startsWith("battery_volts")) {
      gps_sentence.add_battery_field(field_value);
    } else if (field_key.startsWith("cops")) {
      gps_sentence.add_operator_field(field_value);
    } else if (field_key.startsWith("jamm")) {
      gps_sentence.add_jamming_state(field_value);
    } else if (field_key.startsWith("gns")) {
      gps_sentence.decode_gns(field_value);
    } else if (field_key.startsWith("gll")) {
      gps_sentence.decode_gll(field_value);
    } else if (field_key.startsWith("gga")) {
      gps_sentence.decode_gga(field_value);
    } else if (field_key.startsWith("zda")) {
      gps_sentence.decode_zda(field_value);
    } else if (field_key.startsWith("vtg")) {
      gps_sentence.decode_vtg(field_value);
    }
  }
  let rmc_value = values_array.find((item) => item.includes("rmc"));
  if (rmc_value !== void 0) {
    let field_key2 = rmc_value.split(",")[0].trim();
    let field_value2 = rmc_value.replace(field_key2 + ",", "").trim();
    await gps_sentence.decode_rmc(field_value2);
  } else {
    const location_functions = new location_apis();
  ...
  ;
  let psti20_value = values_array.find((item) => item.includes("psti20"));
  if (psti20_value !== void 0) {
    let field_key2 = psti20_value.split(",")[0].trim();
    let field_value2 = psti20_value.replace(field_key2 + ",", "").trim();
    gps_sentence.decode_psti20(field_value2);
  }
  ...
  delete gps_sentence.new_variable_to_be_inserted.metadata.EOF;
  return gps_sentence.new_variable_to_be_inserted;
}

// index.ts
var { Analysis, Utils, Device, Resources } = require("@tago-io/sdk");
...
var catch_time_stamp = (data) => {
  ...
  let final_date = new Date(`${date_elements[2]}-${date_elements[1]}-${date_elements[0]}T${time_elements[0]}:${time_elements[1]}:${time_elements[2]}Z`);
  return final_date;
};
  ...
  for await (let ftp_file of file_list) {
    try {
      ...
      for await (let stu_message of file_content) {
        ...
        if (device && device[0].tags.find((tag) => tag.key === "TYPE" && tag.value === "STX")) {
          console.log("STX");
          const stx_messages = new stx_message();
          let decoded_code = await stx_messages.decode(stu_message, esn_value, time_stamp);
...
async function Decoding_analysis(context, scope) {
  try {
    ...
    if (scope.length !== 0) {
      ...
      let decoded_code = await decode(scope);
